...
export function MarketCard({ market, showCategory = true }: MarketCardProps) {
  ...
  return (
    <Link to={`/trade/${market.id}`} className="block h-full no-underline">
      <div className="h-full p-5 sm:p-6 lg:p-8 bg-neutral-900 rounded-2xl border border-neutral-800 hover:border-neutral-700 flex flex-col transition-colors cursor-pointer">
        {/* Header */}
        ...
        {/* Question */}
        <h3 className="text-sm sm:text-base lg:text-[17px] font-semibold text-white leading-snug sm:leading-relaxed mb-5 sm:mb-6 flex-1 line-clamp-2">
          {market.question}
        </h3>
        ...
        <div className="mb-5 sm:mb-6">
          ...
          <div className="h-1.5 sm:h-2 rounded bg-neutral-800 flex overflow-hidden">
            <div 
        ...
        {/* Footer */}
        <div className="flex justify-between items-center pt-4 sm:pt-5 border-t border-neutral-800">
          <div>
            <p className="text-[10px] sm:text-xs text-neutral-600 mb-1">Total Pool</p>
            <p className="text-sm sm:text-base font-bold text-white">{formatStx(totalPool)}</p>
          </div>
